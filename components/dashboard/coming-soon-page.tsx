...
export function ComingSoonPage({
  ...
  description: string;
  integration: string;
  // When true, renders without the public nav / full-page wrapper so it can sit
  // inside the logged-in app shell.
  embedded?: boolean;
}) {
  const content = (
    <>
      <section className="overflow-hidden rounded-lg border border-zn-line bg-[radial-gradient(circle_at_top,rgba(200,162,74,.14),transparent_44%),#141109] p-8 text-center md:p-14">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Button asChild variant="outline"><Link href={embedded ? "/dashboard" : "/"}>{embedded ? "Back to Dashboard" : "Back to Home"}</Link></Button>
        </div>
      </section>
      <Card className="mt-5">
  ...
  if (embedded) {
    return <div className="mx-auto max-w-4xl">{content}</div>;
  }

  return (
    <div className="min-h-screen bg-zn-black text-zn-parchment">
      <PublicNav />
      <main className="mx-auto max-w-4xl px-4 py-16 md:px-8">{content}</main>
    </div>
  );
}
