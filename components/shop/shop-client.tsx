    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
      ...
      <aside className="space-y-4">
        ...
        <Card id="players">
          ...
          <CardContent className="space-y-3">
            {["RichMiner_", "TradeMaster", "LuckyTotem"].map((name, index) => <div key={name} className="flex items-center justify-between border-b border-white/10 pb-3"><div><p className="font-black">{name}</p><p className="text-xs text-zinc-500">{12 - index * 3} active listings</p></div><Star className="h-4 w-4 text-zn-gold" /></div>)}
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Profession Tips</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm text-zinc-300">
            <p>Blacksmiths should watch netherite, diamonds, and elytra because repair and gear demand spikes before war events.</p>
            <p>Miners can use surplus shulkers to move bulk ore into town banks during resource booms.</p>
          </CardContent>
        </Card>
      </aside>
      {selected && (
