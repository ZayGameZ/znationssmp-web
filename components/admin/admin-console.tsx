...
function renderTab(tab: string, data: {
  ...
  if (tab === "Content") return <Rows rows={[...data.announcements.map((item) => ["Announcement", item.title, item.pinned ? "Pinned" : "Published"]), ...data.events.map((item) => ["Future Event", item.title, item.active ? "Active seed" : "Scheduled seed"])]} />;
